...
export const getForCandidate = async (candidateId) => {
  const res = await axiosInstance.get(`/api/screenings/for-candidate/${candidateId}`);
  return res.data;
};

export const checkAlreadyScreened = async (candidateId, jobId) => {
  const res = await axiosInstance.get(`/api/screenings/check`, { params: { candidateId, jobId } });
