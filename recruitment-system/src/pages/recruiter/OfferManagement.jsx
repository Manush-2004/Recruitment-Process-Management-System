...
import { createOffer, getCandidates, getJobs } from '../../api/recruiterApi';
...
const OfferManagement = () => {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        if (qCandidate && qJob) {
          try {
            const res = await axiosInstance.get(`/api/feedback/summary?candidateId=${qCandidate}&jobId=${qJob}`);
            setSummary(res.data);
          } catch (e) {
  ...
  const fetchSummaryByCandidateJob = async () => {
    ...
    try {
      ...
      console.error('Fetch summary failed', e);
      setMessage(e.response?.status === 403 ? 'You do not have permission to view interview summaries (HR only).' : 'Failed to fetch summary by candidate/job');
    }
  };
  ...
  const onMoveToHr = async () => {
    ...
    try {
      await axiosInstance.post(`/api/candidates/${summary.candidateId}/move-to-hr`);
    ...
    <div className="p-6 max-w-[900px] mx-auto">
      ...
      <div className="bg-white p-4 rounded-ds-card shadow-ds-card">
        <div className="flex gap-2 mb-3">
          ...
          <button onClick={fetchSummaryByInterview} className="px-3 py-1 bg-blue-600 text-white rounded">Fetch by Interview</button>
        </div>

        <div className="flex gap-2 mb-4 items-center">
          <select value={candidateId} onChange={(e) => setCandidateId(e.target.value)} className="p-2 border rounded w-48">
            <option value="">Select candidate</option>
            {candidates.map(c => <option key={c.id} value={c.id}>{c.fullName} ({c.email}) — id:{c.id}</option>)}
          </select>
          <select value={jobId} onChange={(e) => setJobId(e.target.value)} className="p-2 border rounded w-48">
            <option value="">Select job</option>
            {jobs.map(j => <option key={j.id} value={j.id}>{j.title} — id:{j.id}</option>)}
          </select>
          <button onClick={fetchSummaryByCandidateJob} className="px-3 py-1 bg-blue-600 text-white rounded">Fetch by Candidate+Job</button>
        </div>
        ...
        {summary && (
          <div>
            ...
            <p className="text-sm text-ds-text-secondary">Candidate: {summary.candidateName ?? summary.candidateName}</p>
            <p className="text-sm text-ds-text-secondary">Average score: {summary.averageRating ?? summary.averageRating}</p>
            <div className="mt-3 flex gap-2">
              ...
              {hasRole && hasRole('Recruiter') && (
                <button onClick={onMoveToHr} className="px-3 py-1 bg-yellow-600 text-white rounded">Move to HR</button>
              )}
            </div>
          </div>
        )}
