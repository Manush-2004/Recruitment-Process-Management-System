...
const ScreeningList = ({ candidateId, onUpdated }) => {
  ...
  const saveEdit = async (id) => {
    try {
      await reviewerApi.updateScreening(id, { status: editStatus, comments: editComments });
      setEditingId(null);
  ...
  return (
    <div className="bg-white p-4 rounded-ds-card shadow-ds-card">
      {list.length === 0 ? <p className="text-sm text-ds-text-secondary">No screenings for this candidate</p> : (
        <div className="space-y-3">
          {list.map(s => (
            <div key={s.id} className="p-3 border rounded">
              <div className="flex justify-between items-center">
                <div><strong>{s.reviewerName}</strong> <span className="text-sm text-ds-text-secondary">{s.job?.title}</span></div>
                <div className="text-sm">
                  {s.screenedAt ? <span className="text-green-600">{s.status}</span> : <span className="text-yellow-600">Pending</span>}
                </div>
              </div>
              <div className="text-sm text-ds-text-secondary mt-2">{s.comments ?? '—'}</div>
              ...
              {editingId === s.id ? (
                <div className="mt-2 flex gap-2">
                  ...
                  <button onClick={() => saveEdit(s.id)} className="px-3 py-1 bg-blue-600 text-white rounded">Save</button>
                  <button onClick={() => setEditingId(null)} className="px-3 py-1 border rounded">Cancel</button>
                </div>
              ) : (
                ((hasRole('Recruiter') && s.screenedAt) || (hasRole('Reviewer') && user?.fullName === s.reviewerName)) && (
                  <div className="mt-2">
