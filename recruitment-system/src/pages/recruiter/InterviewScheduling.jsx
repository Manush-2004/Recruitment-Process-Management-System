...
import { getCandidates, getJobs, scheduleInterview } from '../../api/recruiterApi';
    ...
    <div className="p-6 max-w-[900px] mx-auto">
      ...
      <form onSubmit={submit} className="bg-white p-4 rounded-ds-card shadow-ds-card">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
          <select value={candidateId} onChange={(e) => setCandidateId(e.target.value)} className="p-2 border rounded">
            <option value="">Select candidate</option>
            {candidates.map(c => <option key={c.id} value={c.id}>{c.fullName}</option>)}
          </select>
          <select value={jobId} onChange={(e) => setJobId(e.target.value)} className="p-2 border rounded">
        ...
        <div className="mb-3">
          ...
          <button type="button" onClick={addInterviewer} className="text-sm text-blue-600">Add interviewer</button>
        </div>

        <div className="mb-3">
          <label className="block text-sm text-ds-text-secondary mb-2">Mode & Link</label>
          <div className="flex gap-2">
            <select value={mode} onChange={(e) => setMode(e.target.value)} className="p-2 border rounded w-40">
              <option>Online</option>
              <option>Onsite</option>
            </select>
            <input placeholder="Meeting link (optional)" value={meetingLink} onChange={(e) => setMeetingLink(e.target.value)} className="p-2 border rounded flex-1" />
          </div>
        </div>
