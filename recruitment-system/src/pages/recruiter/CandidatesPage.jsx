...
const CandidateForm = ({ onSave, onCancel }) => {
  ...
  return (
    <form
      onSubmit={submit}
      className="p-4 bg-white rounded-ds-card shadow-ds-card mb-4"
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-3">
        ...
        <input
          ...
          placeholder="Skills (e.g., JS:3;React:2)"
          className="p-2 border rounded"
        />
      </div>
...
const CandidatesPage = () => {
  ...
  const onBulkUpload = async () => {
    ...
    try {
      ...
      setMessage(
        `Imported ${res.importedCount ?? 0}. Errors: ${res.errors?.length ?? 0}`,
      );
      const cs = await getCandidates();
      setCandidates(cs || []);
    } catch (e) {
    ...
    <div className="p-6 max-w-[1200px] mx-auto">
      <div className="flex items-center justify-between mb-4">
        ...
        <div>
          ...
          >
            {showForm ? "Close" : "Create Candidate"}
          </button>
        </div>
      </div>

      {showForm && (
        <CandidateForm onSave={onSave} onCancel={() => setShowForm(false)} />
      )}
      ...
      <div className="mb-4 p-4 bg-white rounded-ds-card shadow-ds-card">
        ...
        {message && (
          <div className="mt-2 text-sm text-ds-text-secondary">{message}</div>
        )}
      </div>
          ...
          {candidates.map((c) => (
            ...
            >
              ...
              <div className="flex gap-2">
                ...
                <Link
                  to={`/recruiter/screening?candidateId=${c.id}`}
                  className="px-3 py-1 bg-yellow-500 text-white rounded"
                >
                  Assign Screening
                </Link>
                <Link
                  to={`/recruiter/offers?candidateId=${c.id}`}
                  className="px-3 py-1 border rounded"
                >
                  View Feedback
                </Link>
