...
const JobCandidates = () => {
  ...
  return (
    <div className="p-6 max-w-[1200px] mx-auto">
      <h1 className="text-2xl font-semibold mb-4">Candidates for: {job?.title ?? 'Job'}</h1>
      {loading ? <p>Loading…</p> : candidates.length === 0 ? <p className="text-sm text-ds-text-secondary">No linked candidates.</p> : (
        <ul className="space-y-3">
          {candidates.map((c) => {
            const matched = job?.requiredSkills?.filter(rs => c.skills?.some(cs => cs.name === rs.name && cs.years >= rs.minYears)).length ?? 0;
