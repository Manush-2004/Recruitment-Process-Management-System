...
import { getJob } from '../../api/recruiterApi';
...
const JobFeedback = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const jobInterviews = allInterviews.filter(iv => iv.jobId === parseInt(id, 10));
        
        // For each interview, try to fetch feedback summary
        const interviewsWithFeedback = await Promise.all(
          jobInterviews.map(async (iv) => {
            try {
              const feedbackRes = await axiosInstance.get(`/api/feedback/interview/${iv.id}/summary`);
              return { ...iv, feedbackSummary: feedbackRes.data, hasFeedback: true };
            } catch (err) {
  ...
  return (
    <div className="p-6 max-w-[1200px] mx-auto">
      <div className="mb-6">
        <Link to="/recruiter/jobs" className="text-sm text-blue-600 hover:underline mb-2 inline-block">
          ← Back to Jobs
        </Link>
        <h1 className="text-2xl font-semibold">Feedback for: {job?.title ?? 'Job'}</h1>
      ...
      {interviews.length === 0 ? (
        <div className="bg-white p-6 rounded-lg border">
          <p className="text-sm text-ds-text-secondary">No interviews scheduled for this job yet.</p>
        </div>
      ) : (
              ...
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h3 className="font-semibold text-lg">
                    {iv.roundType} Interview - {iv.candidate?.fullName || 'Candidate'}
                  </h3>
                  <p className="text-sm text-ds-text-secondary mt-1">
                    Scheduled: {new Date(iv.scheduledAt).toLocaleString()}
                  </p>
                  <p className="text-sm text-ds-text-secondary">
                    Mode: {iv.mode} {iv.meetingLink && `• Link: ${iv.meetingLink}`}
                  </p>
                  <p className="text-sm text-ds-text-secondary">
                    Panel: {iv.interviewers?.map(i => i.name).join(', ') || 'Not assigned'}
                  </p>
                </div>
              ...
              {iv.hasFeedback ? (
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="text-sm">
                      ...
                      <span className="text-ds-text-secondary ml-2">
                        ({iv.feedbackSummary.totalFeedbacks} feedback{iv.feedbackSummary.totalFeedbacks !== 1 ? 's' : ''})
                      </span>
                    </div>
                  </div>
                    ...
                    {iv.feedbackSummary.feedbacks.map((fb, idx) => (
                      <div key={idx} className="bg-gray-50 p-4 rounded">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span className="text-sm font-semibold">Rating: {fb.overallRating}/5</span>
                        </div>
                ...
                <div className="mt-4 pt-4 border-t">
                  <p className="text-sm text-ds-text-secondary italic">
                    No feedback submitted yet. Waiting for interviewers to provide feedback.
                  </p>
                </div>
              )}
            </div>
