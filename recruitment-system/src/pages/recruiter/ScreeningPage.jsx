...
const ScreeningPage = () => {
    ...
    try {
      ...
      if (candidateId && candidates.length > 0)
        setSelectedCandidate(
          candidates.find((c) => c.id === parseInt(candidateId, 10)),
        ); //
  ...
  const toggleSkill = (idx) =>
    setSkills((s) =>
      s.map((sk, i) =>
        i === idx ? { ...sk, isApproved: !sk.isApproved } : sk,
      ),
    );
  const setYears = (idx, v) =>
    setSkills((s) =>
      s.map((sk, i) =>
        i === idx ? { ...sk, yearsOfExperience: parseInt(v || "0", 10) } : sk,
      ),
    );
  ...
  const submit = async (e) => {
    ...
    if (!selectedCandidate || !selectedJob)
      return alert("Select candidate and job");
    if (assignToReviewer && !selectedReviewerId)
      return alert("Select a reviewer to assign to");
    ...
    if (assignToReviewer) {
      ...
      const reviewer = reviewers.find(
        (r) => String(r.id) === String(selectedReviewerId),
      );
      reviewerName =
        reviewer?.fullName ??
        reviewer?.full_name ??
        reviewer?.email ??
        (reviewer?.id ? String(reviewer.id) : undefined);
    } else {
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
          <select
            ...
            onChange={(e) =>
              setSelectedCandidate(
                candidates.find((c) => c.id === parseInt(e.target.value, 10)),
              )
            }
          ...
          <select
            ...
            onChange={(e) =>
              setSelectedJob(
                jobs.find((j) => j.id === parseInt(e.target.value, 10)),
              )
            }
            className="p-2 border rounded"
          >
        ...
        <div className="mb-3">
          <label className="flex items-center gap-2">
            <input
              type="checkbox"
              checked={assignToReviewer}
              onChange={() => setAssignToReviewer((a) => !a)}
            />{" "}
            Assign to reviewer
          </label>
        </div>
        ...
        <div className="mb-3">
          ...
          {skills.length === 0 ? (
            <p className="text-sm text-ds-text-secondary">
              Select a job to see required skills
            </p>
          ) : (
              ...
              {skills.map((sk, idx) => (
                <div key={idx} className="flex items-center gap-3">
                  ...
                  <input
                    ...
                    onChange={(e) => setYears(idx, e.target.value)}
                    className="w-20 p-2 border rounded"
                  />
                  <label className="flex items-center gap-2">
                    <input
                      type="checkbox"
                      checked={sk.isApproved}
        ...
        <div className="mb-3">
          ...
          {assignToReviewer && (
            <p className="text-sm text-ds-text-secondary mt-1">
              The selected reviewer will complete the screening and set the
              status.
            </p>
          )}
        </div>
        ...
        <div className="flex gap-2">
          <button
            ...
            disabled={
              (assignToReviewer && !selectedReviewerId) ||
              !selectedCandidate ||
              !selectedJob
            }
          >
            {assignToReviewer ? "Assign" : "Submit Screening"}
          </button>
        </div>
      ...
      {selectedCandidate && (
        <div className="mt-6 max-w-[1000px] mx-auto">
          <h2 className="text-xl font-semibold mb-3">
            Screenings for {selectedCandidate.fullName}
          </h2>
          <ScreeningList
            candidateId={selectedCandidate.id}
            onUpdated={() => {
              //
              const reload = async () => {
                const list = await reviewerApi.getForCandidate(
