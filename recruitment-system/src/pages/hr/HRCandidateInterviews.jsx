...
import * as hrApi from '../../api/hrApi.js';
...
const HRCandidateInterviews = () => {
  ...
  const updateInterviewer = (i, field, value) => {
    setInterviewers(s => s.map((it, idx) => idx === i ? { ...it, [field]: value } : it));
  };
  ...
  const handleSchedule = async (e) => {
    ...
    try {
      ...
      const errorMsg = err.response?.data?.message || err.response?.data || 'Failed to schedule interview. Please try again.';
  ...
  const handleUpdateResult = async (interviewId, result) => {
    try {
      ...
      const errorMsg = err.response?.data?.message || err.response?.data || 'Failed to update result';
      setMessage(`Error: ${errorMsg}`);
    ...
    <div className="max-w-4xl mx-auto py-8 px-4">
      <div className="mb-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-semibold">Interviews for {candidate?.fullName || `Candidate #${id}`}</h1>
            <p className="text-sm text-ds-text-secondary mt-1">{candidate?.email}</p>
          </div>
          <button
      ...
      {showScheduleForm && (
        <div className="mb-6 p-4 bg-white border rounded-lg">
          <h2 className="text-lg font-semibold mb-4">Schedule New HR Interview</h2>
            ...
            <div>
                ...
                {candidate?.candidateJobs && candidate.candidateJobs.length > 0 ? (
                  candidate.candidateJobs.map(cj => (
                    <option key={cj.jobId} value={cj.jobId}>
                      {cj.job?.title || `Job #${cj.jobId}`} {cj.job?.companyName ? `- ${cj.job.companyName}` : ''}
                    </option>
                  ))
                ) : (
                  jobs.map(job => (
              ...
              {(!candidate?.candidateJobs || candidate.candidateJobs.length === 0) && (
                <p className="text-xs text-yellow-600 mt-1">
                  ⚠️ This candidate hasn't applied to any specific job. Showing all available jobs.
                </p>
              )}
            </div>
      ...
      <div className="space-y-3">
        ...
        {!interviews || interviews.length === 0 ? (
          <p className="text-sm text-ds-text-secondary p-4 bg-white border rounded">No interviews scheduled yet.</p>
        ) : (
          interviews.map(iv => (
            <div key={iv.id} className="p-4 bg-white border rounded">
              <div className="flex justify-between items-start mb-3">
                ...
                <div className="flex gap-2">
                  <Link to={`/hr/interview/${iv.id}`} className="px-3 py-1 text-sm text-white bg-indigo-600 rounded hover:bg-indigo-700">
                    View Feedback
                  </Link>
                </div>
              </div>
