...
const ScreeningPage = () => {
    ...
    const load = async () => {
      try {
        ...
        if (jobId) {
          ...
          setJob(res2.data);
          setSkills(
            (res2.data.requiredSkills || []).map((s) => ({
              skillName: s.name,
              yearsOfExperience: 0,
              isApproved: false,
            })),
        ...
        if (candidateId && jobId) {
          const already = await api.checkAlreadyScreened(
  ...
  const validate = () => {
    ...
    if (status === "Shortlisted" && !skills.some((s) => s.isApproved))
      return false;
    return true;
  };
  ...
  const submit = async (e) => {
    ...
    if (!validate())
      return alert(
        "Validation failed: ensure reviewer name and at least one approved skill for Shortlisted status.",
      );
    const payload = {
  ...
  const cvUrl = candidate?.documents?.[0]?.filePath
    ? API_BASE_URL + candidate.documents[0].filePath
    : null;
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="md:col-span-2 bg-white p-4 rounded-ds-card shadow-ds-card">
            ...
            <div className="mb-3">
              <label className="block text-sm text-ds-text-secondary">
                (You may edit this screening later from your history)
              </label>
            </div>
          ...
          <div className="bg-white p-4 rounded-ds-card shadow-ds-card">
            ...
            <div className="mb-2">
              <strong>Duplicate Screening:</strong>{" "}
              {duplicate ? (
                <span className="text-red-600">
                  Already screened for this job
                </span>
              ) : (
                <span className="text-green-600">No previous screening</span>
              )}
            </div>
            <div className="mb-3">
              <label className="block text-sm text-ds-text-secondary">
